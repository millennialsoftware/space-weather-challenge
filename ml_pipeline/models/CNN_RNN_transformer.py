"""
CNN_RNN_Transformer.py

Defines a hybrid neural network for time‑series forecasting that stacks:
- 1‑D convolution and max‑pooling layers for local feature extraction.
- A minimal transformer block for self‑attention.
- LSTM and GRU layers for sequential modelling.
- Dense layers for final regression or classification.
- Ray Tune support for basic hyperparameter search.

Utility helpers return the compiled model, common training callbacks, and
Ray‑friendly dictionaries for default parameters and search space.

Key Methods:
    get_model()               returns the compiled Keras model.
    get_callbacks()           provides early‑stopping and LR scheduling.
    get_hyperparameter_space() Ray Tune search grid.
    get_default_params()      baseline hyperparameter set.

Hyperparameter Tuning:
    Exposes settings such as learning_rate, batch_size, convolution filter
    sizes, kernel_size, LSTM/GRU units, dropout rates, dense units, transformer
    heads and dimensions.  See get_hyperparameter_space() for exact ranges.
"""
...
class cnn_lstm_gru_transformer:
    """
